from langgraph.graph import StateGraph
from typing import TypedDict
from agents.researcher import run_researcher
from agents.synthesizer import run_synthesizer
from agents.mapper import run_mapper

class GraphState(TypedDict):
    input: str
    raw_info: dict
    summary: str
    result: dict

def build_langgraph():
    graph = StateGraph(GraphState)

    # Step 1: Researcher node
    def wrapped_researcher(state: GraphState):
        raw = run_researcher(state["input"])
        print("\n [Researcher Output] Raw Info:\n", raw)
        return {"raw_info": raw}

    # Step 2: Synthesizer node
    def wrapped_synthesizer(state: GraphState):
        summary = run_synthesizer(state["raw_info"], state["input"])  
        print("\n [Synthesizer Output] Roadmap Text:\n", summary)
        return {"summary": summary}

    # Step 3: Mapper node
    def wrapped_mapper(state: GraphState):
        graph_data = run_mapper(state["summary"])
        print("\n [Mapper Output] Nodes & Edges:\n", graph_data)
        return {"result": graph_data}

    # Define pipeline
    graph.add_node("research", wrapped_researcher)
    graph.add_node("synthesize", wrapped_synthesizer)
    graph.add_node("map", wrapped_mapper)

    graph.set_entry_point("research")
    graph.add_edge("research", "synthesize")
    graph.add_edge("synthesize", "map")

    return graph.compile()

def autonomous_pipeline(topic):
    langgraph_flow = build_langgraph()
    result = langgraph_flow.invoke({"input": topic})

    # Final debug
    print("\n [Final Result] Returned Graph:")
    print(result["result"])

    return result["result"]


